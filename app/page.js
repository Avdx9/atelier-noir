...
function Nav() {
  ...
  return (
    <>
      ...
      {open && (
        <div style={{position:'fixed',inset:0,background:'rgba(6,4,6,0.98)',zIndex:99,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:'28px'}}>
          {links.map(([l,h])=>(
            <a key={l} href={h} onClick={()=>setOpen(false)} style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'3rem',fontStyle:'italic',fontWeight:300,color:'#F5ECD7',letterSpacing:'0.08em'}}>{l}</a>
          ))}
          <a href="#booking" onClick={()=>setOpen(false)} style={{marginTop:'12px',fontSize:'0.62rem',letterSpacing:'0.25em',textTransform:'uppercase',color:'#060406',background:'#C9A84C',padding:'13px 40px'}}>Reserve Now</a>
        </div>
      )}
    </>
  );
  ...
  return (
    <>
      ...
      <section className="hero">
        <div className="hero-vignette"/><div className="hero-fade"/>
        <div className="hero-content">
          <div className="hero-eyebrow">Mayfair · London · Est. 2012</div>
          <h1 className="hero-title">Atelier<em>Noir</em></h1>
...
const services = [
  ...
  {num:'02',icon:'◈',name:'Bespoke Colour',desc:'Your colour is mixed uniquely for you. No two Atelier Noir colours are the same. From root touch-up to full transformation, each session begins with a full colour consultation.',price:'From £185',dur:'3 hrs'},
  {num:'03',icon:'◇',name:'Balayage & Toning',desc:'Hand-painted, sun-kissed dimension. Our balayage technique creates effortless, lived-in colour that grows beautifully and requires minimal maintenance.',price:'From £220',dur:'3.5 hrs'},
  {num:'04',icon:'⬡',name:'Keratin Treatment',desc:'A smoothing, shine-enhancing treatment that eliminates frizz and dramatically reduces drying time. Results last up to five months.',price:'From £175',dur:'2.5 hrs'},
...
/* ── GALLERY ── */
const shots = [
  {url:'https://images.unsplash.com/photo-1560066984-138daaa70310?w=800&h=1000&fit=crop&q=85',label:'The Salon'},
  {url:'https://images.unsplash.com/photo-1522337360788-8b13dee7a37e?w=600&h=400&fit=crop&q=85',label:'Colour Work'},
  {url:'https://images.unsplash.com/photo-1595476108010-b4d1f102b1b1?w=600&h=400&fit=crop&q=85',label:'The Craft'},
  {url:'https://images.unsplash.com/photo-1562322140-8baeececf3df?w=600&h=500&fit=crop&q=85',label:'Balayage'},
  {url:'https://images.unsplash.com/photo-1500840216050-6ffa99d75160?w=400&h=400&fit=crop&q=85',label:'Bridal'},
  {url:'https://images.unsplash.com/photo-1521590832167-7bcbfaa6381f?w=500&h=400&fit=crop&q=85',label:'Finishing'},
];
      ...
      <div className="gallery-grid">
        {shots.map((s,i)=>(
          <div key={i} className={`g-item reveal d${Math.min(i+1,4)}`}>
            <Image src={s.url} alt={s.label} fill unoptimized style={{objectFit:'cover'}}/>
            <div className="g-overlay"/><div className="g-label">{s.label}</div>
          </div>
        ))}
      </div>
    </section>
    ...
    <section className="section philosophy" id="philosophy" ref={ref}>
      <div className="philosophy-inner">
        <div className="phil-img reveal">
          <Image src="https://images.unsplash.com/photo-1600948836101-f9ffda59d250?w=640&h=840&fit=crop&q=85"
            alt="Head stylist" fill style={{objectFit:'cover',filter:'sepia(12%) contrast(1.08) brightness(0.8)'}} unoptimized/>
        ...
        <div className="phil-text">
          ...
          <p className="phil-body reveal d2">
            Atelier Noir was founded in 2012 with a single conviction: that a truly exceptional hair salon is built on the quality of its relationships, not just the quality of its work. We take on a limited number of clients specifically so that every single appointment receives our complete attention.
          </p>
          <p className="phil-body reveal d3">
            Our team holds advanced colour certifications from London and Paris, with ongoing education embedded into our culture. We use only professional-grade products that have been tested for colour longevity, hair health, and ethical sourcing.
          </p>
...
const reviews = [
  ...
  {text:"The balayage they created for my wedding was exactly what I'd always imagined but never quite been able to describe. They understood instinctively.",name:'Sophie Drake',role:'Bridal Client · June 2024',img:'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=88&h=88&fit=crop',stars:5},
  {text:"I travel from Manchester every six weeks specifically for my appointment here. The quality is worth every journey. There is genuinely nothing comparable.",name:'Priya Anand',role:'Cut & Colour · 2 years',img:'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=88&h=88&fit=crop',stars:5},
  {text:"Marcus understood my hair in a way no stylist ever has. He knew what would work before I'd even finished explaining what I wanted. Remarkable instinct.",name:'James Thornton',role:'Signature Cut · Regular',img:'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=88&h=88&fit=crop',stars:5},
  {text:"The keratin treatment changed everything. Three months later and my morning routine has halved. The team here genuinely care about the long-term health of your hair.",name:'Charlotte Wei',role:'Keratin Treatment',img:'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=88&h=88&fit=crop',stars:5},
];
...
function Testimonials() {
  ...
  return (
    <section className="section testimonials">
      <div style={{maxWidth:'1280px',padding:'0 0 0 64px'}}>
        <div className="eyebrow">Client Stories</div>
        <h2 className="sec-title" style={{marginTop:'16px'}}>Told by <em>those who visit.</em></h2>
      </div>
      <div ref={trackRef} className="testi-track" style={{paddingLeft:'64px',cursor:'grab'}}>
        {reviews.map((r,i)=>(
          <div key={i} className="testi-card" style={{scrollSnapAlign:'start'}}>
            ...
            <div className="testi-author">
              <div className="testi-av"><Image src={r.img} alt={r.name} width={42} height={42} unoptimized/></div>
        ...
        <div className="booking-left">
          ...
          <div className="booking-info">
            {[
              ...
              {icon:'🕘',label:'Hours',val:'Tue – Sat: 9:00am – 7:00pm\nSun: 10:00am – 5:00pm\nMon: Closed'},
              {icon:'📞',label:'Telephone',val:'+44 (0)20 7123 4567'},
              {icon:'✉️',label:'Email',val:'hello@ateliernoir.co.uk'},
              {icon:'⏱',label:'Response Time',val:'We confirm every booking by text within 30 minutes during salon hours.'},
            ].map(d=>(
...
/* ── CTA ── */
function CTA() {
  const R1=280,R2=200,R3=130;
  const mkDots=(r,n,col,op=0.55)=>Array.from({length:n}).map((_,i)=>{const a=(i/n)*Math.PI*2;return{cx:r+r*Math.cos(a),cy:r+r*Math.sin(a),col,op};});
  return (
    <section className="section cta-section">
      ...
      <div className="cta-ring" style={{width:R1*2,height:R1*2}}>
        <svg width={R1*2} height={R1*2} style={{display:'block',animation:'orbitSpin 38s linear infinite'}}>
          <circle cx={R1} cy={R1} r={R1-2} fill="none" stroke="rgba(201,168,76,0.08)" strokeWidth="1"/>
          <circle cx={R1} cy={R1} r={R1-2} fill="none" stroke="rgba(201,168,76,0.18)" strokeWidth="0.5" strokeDasharray="10 22" strokeLinecap="round"/>
          {mkDots(R1,8,'#C9A84C').map((d,i)=><g key={i}><circle cx={d.cx} cy={d.cy} r="3" fill={d.col} opacity={d.op}/><circle cx={d.cx} cy={d.cy} r="8" fill="none" stroke={d.col} strokeWidth="0.5" opacity="0.2"/></g>)}
        </svg>
      </div>
      <div className="cta-ring" style={{width:R2*2,height:R2*2}}>
        <svg width={R2*2} height={R2*2} style={{display:'block',animation:'orbitSpinR 24s linear infinite'}}>
          <circle cx={R2} cy={R2} r={R2-2} fill="none" stroke="rgba(201,168,76,0.06)" strokeWidth="1"/>
          <circle cx={R2} cy={R2} r={R2-2} fill="none" stroke="rgba(201,168,76,0.2)" strokeWidth="0.5" strokeDasharray="5 12"/>
      ...
      <div className="cta-ring" style={{width:R3*2,height:R3*2}}>
        <svg width={R3*2} height={R3*2} style={{display:'block',animation:'orbitSpin 15s linear infinite'}}>
          <circle cx={R3} cy={R3} r={R3-2} fill="none" stroke="rgba(201,168,76,0.07)" strokeWidth="1"/>
      ...
      <h2 className="cta-title" style={{position:'relative',zIndex:2}}>Reserve<br/><em>Your Visit.</em></h2>
      <p className="cta-sub" style={{position:'relative',zIndex:2}}>Available Tuesday through Sunday.<br/>Walk-ins welcome at the styling bar.</p>
      <div style={{display:'flex',gap:'14px',justifyContent:'center',flexWrap:'wrap',position:'relative',zIndex:2}}>
        <a href="#booking" className="btn-gold"><span>Book Online</span></a>
        <a href="tel:+442071234567" className="btn-outline">Call the Salon</a>
      </div>
      <div className="cta-details" style={{position:'relative',zIndex:2}}>
        {[{l:'Address',v:'24 Mayfair Street, London W1K 2AA'},{l:'Tue – Sat',v:'9:00am – 7:00pm'},{l:'Sunday',v:'10:00am – 5:00pm'},{l:'Phone',v:'+44 (0)20 7123 4567'}].map(d=>(
  ...
  return (
    <footer className="footer">
      <div className="footer-top">
        <div>
          ...
          <div className="f-social">
            {[['IG','#'],['FB','#'],['TT','#'],['YT','#']].map(([l,h])=><a key={l} href={h} className="f-s">{l}</a>)}
          </div>
        </div>
        <div className="f-col">
        ...
        <div className="f-col">
          ...
          <div style={{marginTop:'14px',display:'flex',flexDirection:'column',gap:'7px'}}>
            <a href="tel:+442071234567" style={{fontSize:'0.78rem',color:'var(--muted)',fontStyle:'italic'}}>+44 (0)20 7123 4567</a>
            <a href="mailto:hello@ateliernoir.co.uk" style={{fontSize:'0.78rem',color:'var(--muted)',fontStyle:'italic'}}>hello@ateliernoir.co.uk</a>
          </div>
        </div>
        <div className="f-col">
          ...
          <ul>
            {[['Monday','Closed'],['Tue – Wed','9:00 – 19:00'],['Thu – Fri','9:00 – 20:00'],['Saturday','9:00 – 19:00'],['Sunday','10:00 – 17:00']].map(([d,t])=>(
              <li key={d} style={{display:'flex',justifyContent:'space-between',gap:'12px'}}>
                <span style={{fontSize:'0.76rem',color:'var(--muted)'}}>{d}</span>
                <span style={{fontSize:'0.76rem',color:t==='Closed'?'rgba(245,236,215,0.15)':'var(--cream-mid)',fontStyle:'italic'}}>{t}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="footer-bottom">
        ...
        <div className="f-legal"><a href="#">Privacy</a><a href="#">Terms</a><a href="#">Cookies</a></div>
      </div>
    </footer>
  );
}
...
export default function Home() {
  ...
  return (
    <>
      <Nav/>
      <main>
        <Hero/><Marquee/><Services/><Gallery/><Philosophy/>
